...
class BanksTab extends StatefulWidget {
  ...
  const BanksTab({super.key, required this.user});

  @override
  _BanksTabState createState() => _BanksTabState();
}
...
class _BanksTabState extends State<BanksTab> {
  ...
  Future<void> _getToken() async {
    ...
    if (token != null) {
      _token = token;
      setState(() {
        _banksFuture = BankService().fetchBanks(_token!); // Fetch bank data
      });
    } else {
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState ==
                          ...
                          final banks = snapshot.data!
                              .where((bank) =>
                                  bank.category.toLowerCase() == 'bank')
                              .toList();

                          if (banks.isEmpty) {
                            return const Center(
                                child: Text(
                                    'No banks found with category "bank"'));
                          }
