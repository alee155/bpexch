...
class JazzCashTab extends StatefulWidget {
  final UserModel user;

  const JazzCashTab({super.key, required this.user});

  @override
  _JazzcashTabState createState() => _JazzcashTabState();
}

class _JazzcashTabState extends State<JazzCashTab> {
  late Future<List<Bank>> _banksFuture =
      Future.value([]); // Initialize with an empty list
  String? _token;
  Bank? _selectedBank; // To store the selected bank
  ...
  Future<void> _getToken() async {
    ...
    if (token != null) {
      _token = token;
      setState(() {
        _banksFuture = BankService().fetchBanks(
            _token!); // Initialize the future after getting the token
      });
    } else {
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState ==
                          ...
                          final banks = snapshot.data!
                              .where((bank) =>
                                  bank.category.toLowerCase() == 'jazzcash')
                              .toList();

                          if (banks.isEmpty) {
                            return const Center(
                                child: Text('No Jazzcash banks found'));
                          }
                                  ...
                                  child: Container(
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        ...
                                        const Spacer(),
                                        // Checkbox to select the bank
                                        Checkbox(
                                          value: _selectedBank ==
                                              bank, // Check if this bank is selected
